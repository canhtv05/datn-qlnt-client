...
export const useRoom = () => {
  ...
  useEffect(() => {
    if (isRoomError) toast.error(t("rooms.errorFetch"));
  }, [isRoomError, t]);
  ...
  const { data: floorListData, isLoading: isLoadingFloorList } = useQuery<ApiResponse<FloorBasicResponse[]>>({
    ...
    queryFn: async () =>
      (
        await httpRequest.get("/floors/find-all", {
          params: { buildingId: filterValues.buildingId },
        })
      ).data,
  ...
  const roomStats = [
    {
      ...
      value: mapStatistics(statisticsRaw?.data).empty,
    },
    {
      icon: HandCoins,
      label: t("statusBadge.roomStatus.deposit"),
      value: mapStatistics(statisticsRaw?.data).depositing,
    },
    {
      icon: Building2,
      label: t("statusBadge.roomStatus.renting"),
      value: mapStatistics(statisticsRaw?.data).renting,
    },
  ];
  ...
  const handleActionClick = (room: RoomResponse, type: "update" | "delete") => {
    if (type === "update") {
      ...
      openDialog({ id: room.id }, { type: "warn", desc: t(Notice.REMOVE) });
    }
  };
