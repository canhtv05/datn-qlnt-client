...
export const useFloor = () => {
    ...
    const updateFloorMutation = useMutation({
        ...
        mutationFn: async (payload: UpdateFloorValue) =>
            await httpRequest.put(`/floors/${idRef.current}`, payload),
        onError: (error) => {
    ...
    }, [value, updateFloorMutation, clearErrors, queryClient, t, handleZodErrors]);
    ...
    const { data: statistics, isError: errorStatistics } = useQuery<
        ApiResponse<IFloorStatisticsResponse>
    >({
    ...
    const datFloors: StatisticCardType[] = [
        ...
        },
        {
            ...
            label: t("floor.response.active"),
            value: statistics?.data.activeFloors ?? 0,
        },
        {
            icon: XCircleIcon,
            label: t("floor.response.inactive"),
