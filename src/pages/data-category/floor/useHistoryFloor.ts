...
export const useHistoryFloor = () => {
  ...
  const { data, isLoading, isError } = useQuery<ApiResponse<FloorResponse[]>>({
    queryKey: ["floors-cancel", page, size, status, buildingId, id, maxRoom, nameFloor, floorType],
    queryFn: async () => {
    ...
    try {
      await removeFloorMutation.mutateAsync(id, {
        onSuccess: () => {
          ...
          queryClient.invalidateQueries({
            predicate: (query) => Array.isArray(query.queryKey) && query.queryKey[0] === "floors-cancel",
          });
