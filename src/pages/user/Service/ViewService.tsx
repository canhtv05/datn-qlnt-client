...
const serviceStatusColumns: Record<string, string> = {
...
const serviceStatusToString = (status: string) => {
  switch (status) {
    case "DANG_SU_DUNG":
      return "Đang sử dụng";
    case "TAM_DUNG":
...
const RoomServices = () => {
  ...
  const { data, isError, isLoading } = useQuery<
    ApiResponse<ServiceRoomDetailResponse>
  >({
  ...
  useEffect(() => {
    if (isError) {
      toast.error("Không thể tải dịch vụ trong phòng. Vui lòng thử lại sau.");
    }
  }, [isError]);
  ...
  return (
    <Overlay>
      <div className="bg-background rounded-md w-full max-h-[90vh] overflow-y-auto p-4 flex flex-col gap-4">
        <div className="flex justify-between items-center mb-3">
          <h3 className="font-semibold text-lg">
            Dịch vụ phòng: {room?.roomCode || NA}
          </h3>
          <button
            onClick={() => navigate(-1)}
        ...
        {isLoading ? (
          <div className="flex gap-4">
            {Array.from({ length: 2 }).map((_, idx) => (
              <div
                key={idx}
                className="flex-1 min-w-[250px] bg-gray-200 rounded-xl animate-pulse h-40"
              />
            ))}
          </div>
        ) : !room?.services || room.services.length === 0 ? (
          <div className="text-center py-10 text-gray-500">
            Chưa có dịch vụ nào trong phòng
          </div>
        ) : (
          <div className="flex gap-4 overflow-x-auto">
            {Object.entries(serviceStatusColumns).map(([statusKey, label]) => (
              <div
                key={statusKey}
                    ...
                    >
                      ...
                      <span
                        className={`inline-block px-3 py-1 rounded-full text-xs font-semibold w-fit ${
                          serviceStatusColorMap[service.serviceRoomStatus] ||
                          "bg-gray-400 text-white"
                        }`}
                      >
                        {serviceStatusToString(service.serviceRoomStatus)}
                      </span>
                      {service.description && (
